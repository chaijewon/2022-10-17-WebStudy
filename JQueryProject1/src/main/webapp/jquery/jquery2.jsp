...
$(function(){
	...
	/*
	   ...
	   이벤트 사용법 
	     ...
	     $('선택자').click(function(){처리})
	     $('선택자').on('click',function(){처리})
	     hover : img , input 
	     keyup : 키보드 입력 
	     change: <select>
	     ---------------------------------
	     show(),hide()
	     show(2000) , show(fast) => 속도 (이펙트)
	     
	*/
	
	$('#keyword').keyup(function(){
		let k=$('#keyword').val(); // 입력된 값을 가지고 온다 
		$('#user-table tbody tr').hide() // 감추기 
		let temp=$('#user-table tbody tr td:nth-child(5n+3):contains("'+k+'")')
		$(temp).parent().show() // 보여준다 
		// td찾고 => 출력 tr => CSS를 이용
	})
})
</script>
  ...
  <div class="container">
   ...
   <div class="row">
     <table class="table">
      <tr>
       <td>
        <input type=text size=30 class="input-sm" id="keyword" autocomplete="off" placeholder="검색어 입력하세요">
       </td>
      </tr>
     </table>
